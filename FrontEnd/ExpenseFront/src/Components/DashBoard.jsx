  ...
  useEffect(() => {
    const income1 = async () => {
      try {
        ...
        const result2 = await axios.get(`${apiUrl}transactions/income`, config);
    ...
    const minMax2 = async () => {
      ...
      try {
        const result2 = await axios.get(
          `${apiUrl}transactions/expense/maxmin`,
          hd
        );
    ...
    const recent = async () => {
      try {
        ...
        const data3 = await axios.get(
          `${apiUrl}transactions/expense/recent`,
          config
        );
        if (data3.data.data) {
      ...
      <div className="main-dashboard">
        <div className="child-1">
          ...
          <div className="difference-parent">
            <Card
              title={"Income Left"}
              value={`$ ${income - expense > 0 ? income - expense : 0}`}
            />
          </div>
        </div>
        <div className="child-2">
          <SnackBarHeader
            val={"Salary"}
            icon={<FaSackDollar className="icon-2" />}
          />
          <Snackbar min={`${min}`} max={`${max}`} />
          <SnackBarHeader
            val={"Expense"}
            icon={<GiWallet className="icon-1" />}
          />
                ...
                return (
                  <div className="recent-table">
                    <span className="adjust">
                      {element.dataType === "Income" ? (
                        <p className="income-recent">{element.title}</p>
                      ) : (
                        <p className="recent-expense">{element.title}</p>
                      )}
                    </span>
                    <span className="adjust">
                      {element.dataType === "Income" ? (
                        <p className="income-recent2">{+element.amount}</p>
                      ) : (
                        <p className="recent-expense2">{-element.amount}</p>
                      )}
                    </span>
                  </div>
                );
